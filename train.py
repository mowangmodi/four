...
def main(hparams, config):
    ...
    config.TRAINER.LR = config.TRAINER.CANONICAL_LR
    ...
    checkpoint_callback = \
        ModelCheckpoint(dirpath=os.path.join(f'ckpts/{hparams.exp_name}/save'),
                        monitor='val/psnr',
                        mode='max',
                        save_top_k=-1)
    ...
    trainer = Trainer(max_epochs=hparams.num_epochs,
                      ...
                      gpus=[1], #hparams.num_gpus
                      num_nodes=hparams.num_nodes,
                      accelerator='ddp' if hparams.num_gpus>1 else None,
                      num_sanity_val_steps=0, # -1
                      val_check_interval=config.TRAINER.VAL_FREQ,  # config.TRAINER.VAL_FREQ=10000
                      benchmark=True,
