...
class NeuconWSystem(LightningModule):
    def __init__(self, hparams, config, caches):
        ...
        # self.models = {"neuconw": self.neuconw, "nerf": self.nerf, "tcnn": self.neuconw.tcnn_encoding}
    ...
    def forward(self, rays, ts, label):
        ...
        #train before update coarse2fine
        self.neuconw.set_active_levels(self.global_step)
        self.neuconw.set_normal_epsilon(self.global_step)
        self.neuconw.get_curvature_weight(current_iteration=self.global_step, init_weight=self.config.NEUCONW.LOSS.curvature_weight)
    ...
    def surface_selection(self, train_level, threshold, device=0, chunk=65536, neuconw=None, global_step=0):
        ...
        out_chunks = []
        NELO_para = neuconw.NELO_para
        # neuconw.set_active_levels(global_step)
        
        for i in tqdm(range(0, B, chunk), disable=local_rank != 0):
            points_3D = local_xyz_[i : i + chunk].to(device)
            points_enc = neuconw.encode(points_3D).reshape(-1, 131)   # 
            new_sdf = self.renderer.sdf(points_enc.cuda())
    ...
    def octree_update(
        ...
        sparse_pc_sfm, train_voxel_size = self.surface_selection(
            train_level, threshold, device, chunk, neuconw=self.neuconw, global_step=self.global_step
        )
