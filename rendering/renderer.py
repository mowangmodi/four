...
class NeuconWRenderer:
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, new_z_vals, sdf, last=False, neuconw=None, NELO_para=None):#, global_step=0):
        batch_size, n_samples = z_vals.shape
        ...
        if not last:
            ...
            points_enc = neuconw.encode(pts).reshape(-1, 131)   
            new_sdf = self.sdf(points_enc).reshape(_n_rays, _n_samples)
    ...
    def sparse_sampler(self, rays_o, rays_d, near, far, perturb, neuconw=None, global_step=0):
        ...
        if self.n_importance > 0:
            ...
            with torch.no_grad():
                ...
                points_enc = neuconw.encode(pts).reshape(-1, 131)
                sdf = self.sdf(points_enc).reshape(batch_size, self.n_samples)
    ...
    def render_core(
        ...
        static_out = self.neuconw(torch.cat(inputs, -1))#, global_step)
        rgb, inv_s, sdf, gradients, hessians = static_out
    ...
    def render(
        ...
        with torch.no_grad():
            n_samples, z_vals, z_vals_outside, sample_dist = self.sparse_sampler(
                rays_o, rays_d, near, far, perturb, neuconw#, global_step
            )
    ...
    def rgb(self, pts, rays_d, a_embedded):
        ...
        # self.neuconw.set_active_levels(global_step)
        # self.neuconw.set_normal_epsilon(global_step)
