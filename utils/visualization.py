...
def extract_mesh(dim, chunk, scene_radius, scene_origin, origin=None, radius=1.0,
                       with_color=False, embedding_a=None, chunk_rgb=256, sparse_data=None, renderer=None, neuconw=None, global_step=0, NELO_para=None, device="cuda:0",ckpt_path=None):
    if origin is None:
        origin = [0, 0, 0]
    ...
    ###nelo parameter
    nelo = NELO_para.model.object.sdf
    
    if local_rank == 0:
        print("start evaluating sdf...")

    ####if have ckpt_path get global_stpe
    neuconw.set_active_levels(global_step)
    neuconw.set_normal_epsilon(global_step)
    if ckpt_path is not None:
        ...
        neuconw.set_normal_epsilon(global_step)
    with torch.no_grad():
        # neuconw.set_active_levels(2340653)
        # neuconw.set_normal_epsilon(global_step)
        B = local_xyz_.shape[0]
        out_chunks = []
        for i in tqdm(range(0, B, chunk), disable=local_rank!=0):
            ###hash table (B,131)
            local_xyz_chunk = local_xyz_[i:i + chunk].to(device)  
            points_enc = neuconw.encode(local_xyz_chunk)
            new_sdf = renderer.sdf(points_enc.reshape(-1, 131)).cuda()
    ...
    if local_rank == 0:
        ...
        # verts, faces, norms, vals = measure.marching_cubes(sdf, level=None, mask=mask_dense)
        if local_rank == 0:
