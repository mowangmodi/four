...
from utils.loss_fun import curvature_loss
class NeuconWLoss(nn.Module):
    ...
    """
    def __init__(self, coef=1, igr_weight=0.1, mask_weight=0.1, depth_weight=0.1, floor_weight=0.01, curvature_weight=0.0005, config=None):
        super().__init__()
        self.coef = coef
    ...
    def forward(self, inputs, targets, masks=None):
        ...
        ret['hessians'] = self.curvature_weight * curvature_loss(hessian=inputs['hessians'])
